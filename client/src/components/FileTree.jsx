...
function FileTreeNode({
    ...
    const isCut =
        clipboard && clipboard.cut && clipboard.originalId === node.id;
    ...
    function handleClick(e) {
        ...
        // If this node is currently being renamed, ignore single clicks so that
        // clicks inside the rename input don't re-select the node.
        if (renameId === node.id) {
            e.stopPropagation();
            return;
        }
            ...
            >
                ...
                {renameId === node.id ? (
                    <input
                        ...
                        // Prevent clicks/presses inside the input from triggering the parent's
                        // click handler (which would re-select or toggle the node).
                        onMouseDown={e => e.stopPropagation()}
                        onClick={e => e.stopPropagation()}
                    />
                ) : (
                    <span className="file-node__name node-name">
                        {node.name}
                    </span>
                )}
...
function FileTree({
    ...
    function handleMenuAction(action) {
        ...
        if (
            !menu.parent &&
            (action === "delete" || action === "copy" || action === "cut")
        ) {
        ...
        if (action === "copy") {
            ...
            if (clipboard.cut && clipboard.originalId) {
                const movingMeta = getNodeMeta(
                    internalTree,
                    clipboard.originalId
                );
                if (
                    movingMeta &&
                    movingMeta.node &&
    ...
    function commitRename(nodeId, newName) {
        ...
        node.name = exists
            ? resolveName(siblings, newName, node.type)
            : newName;
        setInternalTree([...internalTree]);
                    ...
                    >
                        ...
                        <div className="modal-menu__actions">
                         ...
                         but we keep defensive checks in handleMenuAction */}
                            <button
                                style={{
                                    color: !openMenu.parent ? "grey" : null
                                }}
