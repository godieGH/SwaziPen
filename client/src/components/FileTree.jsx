...
function resolveName(children, name, type) {
  let base = name,
    i = 1;
  let exists = () => children.some(child => child.name === name && child.type === type);
  while (exists()) {
...
const defaultFiles = [
  {
    ...
    children: [
      ...
      { id: "src-1", name: "src", type: "folder", children: [{ id: "index-1", name: "index.js", type: "file" }] }
    ]
  }
];
...
function insertNodeIntoArray(destinationArray, node, insertIndex = undefined) {
  ...
  if (insertIndex === undefined || insertIndex === null || insertIndex > destinationArray.length) {
...
function FileTreeNode({
      ...
      if (node.type === "folder") {
        ...
        if (typeof onSelect === "function") {
          ...
          const payload = {
            ...
            path: meta ? meta.path : [{ id: node.id, name: node.name, type: node.type }],
  ...
  function onRenameCommitLocal() {
    if (typeof commitRename === "function") {
      ...
      const siblings = actualParent ? (actualParent.children || []).filter(c => c.id !== node.id) : tree.filter(c => c.id !== node.id);
      const exists = siblings.some(child => child.name === renameValue && child.type === actualNode.type);
      actualNode.name = exists ? resolveName(siblings, renameValue, actualNode.type) : renameValue;
      setTree([...tree]);
      setRenameId(null);
      setRenameValue("");
    }
  ...
  return (
    <div className={`file-node ${isRoot ? "root-node" : ""}`} tabIndex={0} aria-label={node.name} role="treeitem">
      <div
        className={`file-node__label ${node.type === "folder" ? "folder" : "file"} ${isRoot ? "header" : ""} ${isSelected ? "selected" : ""} ${isCut ? "cut" : ""} ${isHighlighted ? "highlight" : ""}`}
        onClick={handleClick}
        onDoubleClick={handleDoubleClick}
      >
        {node.type === "folder" ? (open ? <FaRegFolder /> : <FaFolder />) : <FaFile color="#929292" />}
        ...
        <button className="file-node__menu-btn" onClick={handleMenuBtn} aria-label="Open menu" tabIndex={-1}>
          <FiMoreVertical />
        </button>
      </div>
...
function FileTree({
  ...
  function handleMenuAction(action) {
    ...
    if (action === "copy") {
      ...
      notify("copy", { node: deepClone(node), parent: deepClone(parent), path, index });
    } else if (action === "cut") {
      setClipboard({ node: deepClone(node), cut: true, originalId: node.id });
      notify("cut", { node: deepClone(node), parent: deepClone(parent), path, index });
      ...
      if (clipboard.cut && clipboard.originalId) {
        ...
        if (movingMeta && movingMeta.node && movingMeta.node.type === "folder") {
          const descendantIds = [];
          function buildIds(n) {
      ...
      const newPath = (targetMeta ? targetMeta.path : []).concat({ id: toPaste.id, name: toPaste.name, type: toPaste.type });

      notify("paste", { node: deepClone(toPaste), parent: deepClone(targetNode), path: newPath, index: destinationArray.length - 1, clipboard: deepClone(clipboard) });
      ...
      if (selectedId && removed && (removed.id === selectedId || (removed.type === "folder" && getNodeMeta([removed], selectedId)))) {
        setSelectedId(null);
      }
      notify("delete", { node: deepClone(removed), parent: deepClone(parent), path, index });
    } else if (action === "newfile" || action === "newfolder") {
      const targetMeta = getNodeMeta(internalTree, id);
      const targetNode = targetMeta ? targetMeta.node : null;
      const parentForNew = targetNode && targetNode.type === "folder" ? targetNode : menu.parent;
      const container = parentForNew ? (parentForNew.children = parentForNew.children || []) : internalTree;
      if (action === "newfile") {
        const newNode = { id: uuid(), name: resolveName(container, "untitled.txt", "file"), type: "file" };
        ...
        notify("newfile", { node: deepClone(newNode), parent: deepClone(parentForNew), path: targetMeta ? targetMeta.path.concat({ id: newNode.id, name: newNode.name, type: newNode.type }) : [], index: container.length - 1 });
      } else {
        const newNode = { id: uuid(), name: resolveName(container, "untitled", "folder"), type: "folder", children: [] };
        container.push(newNode);
        setInternalTree([...internalTree]);
        const ancestorIds = targetMeta ? targetMeta.path.map(p => p.id) : [];
        if (parentForNew) expandFoldersByIds(ancestorIds);
        ...
        if (highlightTimeoutRef.current) clearTimeout(highlightTimeoutRef.current);
        highlightTimeoutRef.current = setTimeout(() => setHighlightedId(null), 2500);
        notify("newfolder", { node: deepClone(newNode), parent: deepClone(parentForNew), path: targetMeta ? targetMeta.path.concat({ id: newNode.id, name: newNode.name, type: newNode.type }) : [], index: container.length - 1 });
      }
    }
  ...
  function commitRename(nodeId, newName) {
    ...
    const siblings = parent ? (parent.children || []).filter(c => c.id !== nodeId) : internalTree.filter(c => c.id !== nodeId);
    const exists = siblings.some(child => child.name === newName && child.type === node.type);
    ...
    notify("rename", { node: deepClone(node), parent: deepClone(parent), path, index });
  }
        ...
        >
          ...
          >
            ...
            <div className="modal-menu__actions">
              ...
              <button onClick={() => handleMenuAction("copy")}>Copy</button>
              <button onClick={() => handleMenuAction("cut")}>Cut</button>

              {/* Show paste only for folders (target must be a folder to paste into) */}
              {openMenu.type === "folder" && (
                <button onClick={() => handleMenuAction("paste")} disabled={!clipboard}>
              ...
              {openMenu.type === "folder" && (
                <>
                  <button onClick={() => handleMenuAction("newfile")}>New File</button>
                  <button onClick={() => handleMenuAction("newfolder")}>New Folder</button>
                </>
              )}
            </div>
          </div>
