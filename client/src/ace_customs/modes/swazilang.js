...
ace.define(
    ...
    function (require, exports, module) {
        ...
        var SwaziLangHighlightRules = function () {
            ...
            // --- Improved template literal handling using push/pop (stack) like Ace's JS rules.
            // This replaces the earlier handcrafted nested states with a stack-based approach
            // so arbitrary nesting of `${ ... ${ ... } ... }` and nested backticks is handled.

            // Inject the brace-level handler & the push-based template rule at the front of start.
            // The onMatch handler for braces manipulates the tokenizer stack so that entering and
            // leaving `{}` sequences inside expressions will return to the correct state, and
            // will emit paren.quasi.end for '}' that close template expressions.
            this.$rules.start.unshift(
                {
                    regex: "[{}]",
                    onMatch: function (val, state, stack) {
                        // nextState is "start" (see nextState below)
                        this.next = val == "{" ? this.nextState : "";
                        if (val == "{" && stack.length) {
                            // entering a new brace-level inside some state:
                            // push the state's name so we can return to it later
                            stack.unshift("start", state);
                        } else if (val == "}" && stack.length) {
                            // leaving a brace-level: pop and restore previous state
                            stack.shift();
                            this.next = stack.shift();
                            // if the restored state is a string/template-like state,
                            // signal that this '}' closed a template expression
                            if (this.next.indexOf("string") != -1 || this.next.indexOf("jsx") != -1 || this.next.indexOf("quasi") != -1)
                ...
                // push-based template literal rule
                {
                    ...
                    push: [
                        // allow escapes
                        { token: "constant.language.escape", regex: "\\\\." },
                        // when we see ${, push the main language state so expressions are tokenized normally
                        { token: "paren.quasi.start", regex: /\$\{/, push: "start" },
                        // closing backtick -> pop back to previous state
                        { token: "string.quasi.end", regex: /`/, next: "pop" },
                        // default content inside template literal
