...
router.get("/", (req, res) => {
    ...
    res.json(
        typeof req.initialFileTree !== "undefined" ? req.initialFileTree : null
    );
});
...
router.get("/filetree", (req, res) => {
    ...
    res.json({
        filetree: [
            typeof req.initialFileTree !== "undefined"
                ? req.initialFileTree
                : null
        ]
    });
...
router.post("/get/file/content", async (req, res) => {
    try {
        ...
        const { resolved: resourcePath } = resolveAndValidateUnderBase(
            req.scannedWD,
            names
        );
        ...
        if (watcher) {
            try {
                ...
                console.error("Error closing old watcher:", err);
            }
            watcher = null;
        }

        // Create new watcher for the new file
        watcher = chokidar.watch(resourcePath, {
            persistent: true,
            ignoreInitial: true
        });
        watcher.on("all", async (evt, changedPath) => {
            if (evt === "change") {
                const cnt = await fs.readFile(changedPath, "utf8");
                const td = await scanProject(req.scannedWD);
        ...
        sessionMap.set("previousOpenedFile", req.body);
...
router.get("/get/previous/opened/file", async (req, res) => {
    try {
        if (sessionMap.has("previousOpenedFile")) {
            return res.json(
                sessionMap.get("previousOpenedFile")
            );
        }
        res.json(null);
    } catch (e) {}
...
router.post("/save/file", async (req, res) => {
    try {
        ...
        const names = extractNamesFromPathArray(
            (treeFileData && treeFileData.path) || []
        );

        const { resolved: saveToPath } = resolveAndValidateUnderBase(
            req.scannedWD,
            names
        );
...
router.post("/create/new/file", async (req, res) => {
    try {
        ...
        const { resolved: newFilePath } = resolveAndValidateUnderBase(
            req.scannedWD,
            names
        );
...
router.post("/create/new/folder", async (req, res) => {
    try {
        ...
        const { resolved: newFolderPath } = resolveAndValidateUnderBase(
            req.scannedWD,
            names
        );
...
router.post("/remove/dir/file/", async (req, res) => {
    try {
        ...
        const { baseRoot, resolved: reSourceToRemovePath } =
            resolveAndValidateUnderBase(req.scannedWD, names);
        ...
        if (sessionMap.has("previousOpenedFile")) {
            if (
                JSON.stringify(
                    sessionMap.get("previousOpenedFile").path
                ) === JSON.stringify(nodePath)
            ) {
                sessionMap.delete("previousOpenedFile");
            }
        }
...
router.post("/rename/dir/file/", async (req, res) => {
    try {
        ...
        const dest = path.join(
            parentDir,
            node && node.name ? String(node.name) : path.basename(src)
        );
        ...
        if (
            ...
            return res
                .status(400)
                .json({
                    error: `Source path is outside base root: ${srcNorm}`
                });
        }
        if (
            path
                .relative(baseRoot, destNorm)
                .split(path.sep)
                .some(p => p === "..")
        ) {
            return res.status(400).json({
                error: `Destination path is outside base root: ${destNorm}`
            });
        }
        ...
        if (!existsSrc) {
            return res
                .status(400)
                .json({ error: `Source path does not exist: ${srcNorm}` });
        }
        ...
        if (srcNorm === destNorm) {
            safeLog(
                req,
                chalk.yellow,
                "Source and destination are the same. No rename performed."
            );
            return res.status(200).json({
                message:
                    "Source and destination are the same. No rename needed."
            });
        }
        ...
        sessionMap.set("deletedFile", srcNorm)
        ...
        safeLog(
            req,
            chalk.green,
            `Rename successful: ${srcNorm} -> ${destNorm}`
        );
...
router.post("/paste", async (req, res) => {
    try {
        ...
        if (!node || !clipboardPath || !operation || !srcPath || !destPath) {
            return res
                .status(400)
                .json({ error: "Missing required clipboard information" });
        }
        ...
        const { baseRoot, resolved: srcResolved } = resolveAndValidateUnderBase(
            req.scannedWD,
            srcNames
        );
        const { resolved: destResolved } = resolveAndValidateUnderBase(
            req.scannedWD,
            destNames
        );

        safeLog(req, chalk.blueBright, "Paste requested", {
            operation,
            srcResolved,
            destResolved
        });
        ...
        if (!srcExists) {
            return res
                .status(404)
                .json({ error: `Source does not exist: ${srcResolved}` });
        }
        ...
        if (operation === "cut") {
            ...
            if (srcNorm === destNorm) {
                safeLog(
                    req,
                    chalk.yellow,
                    "Cut: source and destination are the same, skipping move"
                );
                return res.status(200).json({
                    message:
                        "Source and destination are the same. No action taken."
                });
            }
            ...
            safeLog(
                req,
                chalk.green,
                `Moved (cut) ${srcResolved} → ${destResolved}`
            );
            return res.status(200).json({
            ...
            safeLog(
                req,
                chalk.green,
                `Copied ${srcResolved} → ${destResolved}`
            );
            return res.status(200).json({
