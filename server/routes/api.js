...
router.post("/rename/dir/file/", async (req, res) => {
   try {
      ...
      if (
         ...
         return res
            .status(400)
            .json({ error: `Source path is outside base root: ${srcNorm}` });
      }
      if (
         path
      ...
      if (srcNorm === destNorm) {
         ...
         return res
            .status(200)
            .json({
               message: "Source and destination are the same. No rename needed."
            });
      }
      ...
      console.error(
         chalk.red(
            `Rename error: ${err && err.message ? err.message : String(err)}`
         )
      );
...
router.post("/paste", async (req, res) => {
   try {
      ...
      console.error(
         chalk.red(
            `Error in /paste: ${err && err.message ? err.message : String(err)}`
         )
      );
