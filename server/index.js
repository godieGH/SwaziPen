  ...
  io.on("connection", (socket) => {
    ...
    socket.on("filetree:requestRefresh", async () => {
      try {
        ...
        socket.emit("filetree:error", { message: "Refresh failed", error: "" + err });
      }
    });
    ...
    if (!scanScheduled) {
      ...
      setTimeout(async () => {
        ...
        try {
          ...
          io.emit("filetree:error", { message: "Scan failed", error: "" + err });
...
server.listen(PORT, () => {
  console.log(
    chalk.green.bold('✔') + ' ' +
    chalk.cyan.bold('SwaziPen server started') + ' at port: ' +
  ...
  console.log(
    '  ' + chalk.gray('Press CTRL‑C to shut down')
  );
});
