...
export default function attachTerminalHandlers(socket, scannedWD) {
  ...
  async function spawnRunPty({ id, cols = 80, rows = 24, code = "", filename = "run.sl" }) {
    ...
    const tmpFile = path.join(tmpDir, uniqueName);

    try {
      await fs.writeFile(tmpFile, code, "utf8");
    } catch (err) {
      socket.emit("terminal:error", { id, message: "Failed to write temp file: " + err.message });
      return;
    }
    ...
    try {
      ...
      const p = pty.spawn("swazi", [tmpFile], {
        name: "xterm-color",
        cols,
        rows,
      ...
      terminals.set(id, { pty: p, tmpFile });
      socket.emit("terminal:created", { id });
    } catch (err) {
      // cleanup on spawn failure
      try { await fs.remove(tmpFile); } catch (e) {}
  ...
  socket.on("terminal:create", async (opts = {}) => {
    ...
    if (mode === "run") {
      ...
      await spawnRunPty({ id, cols, rows, code: opts.code || "", filename: opts.filename || "run.sl" });
    } else if (mode === "repl") {
      spawnReplPty({ id, cols, rows });
    } else {
      socket.emit("terminal:error", { id, message: "Unsupported terminal mode" });
    }
  });
  ...
  socket.on("disconnect", () => {
    ...
    terminals.forEach((store) => {
      ...
      if (store.tmpFile) {
        try { fs.removeSync(store.tmpFile); } catch (e) {}
      }
    });
    terminals.clear();
  });
